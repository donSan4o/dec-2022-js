...
coursesAndDurationArray.forEach((item) => {
    course.innerHTML += `<li>Title: ${item.title} Month Duration: ${item.monthDuration}</li>`;
});
...
blockItem.classList.add('item-list');

coursesAndDurationArrayNew.forEach((item) => {
    blockItem.innerHTML += `<div class="item"><h1 class="heading">${item.title}</h1><p class='description'>${item.monthDuration}</p></div>`;
});
...
coursesArray.forEach((course) => {
    ...
    courses.innerHTML += `
        <div class="course-header">
            <h2>${course.title}</h2>
            <span>Month Duration: ${course.monthDuration}</span>
            <span>Hour Duration: ${course.hourDuration}</span>
        </div>`;
    ...
    course.modules.forEach((module) => {
        modulesList.innerHTML += `
            <li>${module}</li>
        `;
        courses.appendChild(modulesList);
    });
});
